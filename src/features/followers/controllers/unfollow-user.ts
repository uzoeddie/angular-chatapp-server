...
export class Remove {
  public async following(req: Request, res: Response): Promise<void> {
    ...
    const response: [this, BulkWriteOpResultObject] = await Promise.all([unFollowing, users]);

    if (response) {
      userQueue.addUserJob('updateUserFollowersInCache', { key: `${req.params.followerId}`, prop: 'followersCount', value: -1 });
      userQueue.addUserJob('updateUserFollowersInCache', {
        key: `${req.currentUser?.userId}`,
        prop: 'followingCount',
        value: -1
      });
      followerQueue.addFollowerJob('removeFollowerFromCache', {
        key: `${req.currentUser?.userId}`,
