...
export class GetUser {
    public async all(req: Request, res: Response): Promise<void> {
        ...
        let allUsers: IUserDocument[] | any;
        ...
        const followers: Query<Pick<IFollowerDocument, "_id" | "createdAt" | "followerId" | "followeeId">[]> = FollowerModel.find({ followerId: req.currentUser?.userId })
            .lean()
            ...
            const response: [IUserDocument[], any[]] = await Promise.all([allUsers, followers]);
    ...
    public async profile(req: Request, res: Response): Promise<void> {
        ...
        const existingUser: object = (cachedUser ? cachedUser : await UserModel.findOne({ _id: req.currentUser?.userId }).lean()) as object;
    ...
    public async username(req: Request, res: Response): Promise<void> {
        ...
        const existingUser: IUserDocument | Query<Pick<IUserDocument, any>> = (cacheResponse[0] ? cacheResponse[0] : UserModel.findOne({ username }).lean()) as IUserDocument | Query<Pick<IUserDocument, any>>;
        ...
        const response: [any, IPostDocument[]] = await Promise.all([ existingUser, userPosts ]);
