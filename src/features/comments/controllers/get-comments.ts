...
export class GetPost {
    public async comments(req: Request, res: Response): Promise<void> {
        ...
        const comments: ICommentDocument[] = await Helpers.getPostComments({ postId: mongoose.Types.ObjectId(postId) }, 0, 100, { createdAt: -1 });
    ...
    public async reactions(req: Request, res: Response): Promise<void> {
        ...
        // }
        let query = { postId: mongoose.Types.ObjectId(postId) };
        const reactions: [IReactionDocument[], number] = await Helpers.getPostReactions(query, 0, 100, { createdAt: -1 });
    ...
    public async singleComment(req: Request, res: Response): Promise<void> {
        ...
        const comments: ICommentDocument[] = await Helpers.getPostComments({ _id: mongoose.Types.ObjectId(commentId) }, 0, 1, { createdAt: -1 });
    ...
    public async singleReaction(req: Request, res: Response): Promise<void> {
        ...
        let query = { _id: mongoose.Types.ObjectId(reactionId) };
        const reactions: [IReactionDocument[], number] = await Helpers.getPostReactions(query, 0, 1, { createdAt: -1 });
