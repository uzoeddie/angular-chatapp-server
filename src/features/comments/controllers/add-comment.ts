...
export class Add {
    ...
    public async comment(req: Request, res: Response): Promise<void> {
        ...
        const posts: Query<any> = PostModel.updateOne({ _id: req.body.postId },  { $inc: { comments: 1 }});
        const user: Promise<IUserDocument> = getUserFromCache(req.body.userTo);
        const response: [ICommentDocument, Query<any>, any, IUserDocument] = await Promise.all([
            comments, 
            posts, 
        ...
        if (response) {
            postQueue.addPostJob('updateSinglePostInRedis', { type: 'comments', key: req.body.postId, value: `${response[2]?.comments}`, username: `${req.currentUser?.username}` });
        }
