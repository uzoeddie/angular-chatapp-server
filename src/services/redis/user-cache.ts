...
export function saveUserToRedisCache(key: any, userId: number, createdUser: IUserDocument): Promise<string> {
    const {
        _id,
        uId,
        username,
    ...
    const firstList: string[] = ['_id', `${_id}`, 'uId', `${uId}`, 'username', `${username}`, 'email', `${email}`, 'avatarColor', `${avatarColor}`, 'createdAt', JSON.stringify(createdAt), 'birthDay', JSON.stringify(birthDay), 'postCount', JSON.stringify(postCount)];
    ...
    const fourthList: string[] = ['followersCount', `${followersCount}`, 'followingCount', `${followingCount}`, 'notifications', JSON.stringify(notifications), 'profilePicture', JSON.stringify(profilePicture)];
    const dataToSave: string[] = [...firstList, ...secondList, ...thirdList, ...fourthList];
    return new Promise((resolve, reject) => {
        client.hmset(`users:${key}`, dataToSave, (error, response) => {
            if (error) {
                reject(error);
            }
...
export function getUserFromCache(key: string): Promise<IUserDocument> {
    return new Promise<any>((resolve, reject) => {
        client.hgetall(`users:${key}`, (error, response) => {
            ...
            // response.profilePicture = JSON.parse(response.profilePicture);
            resolve(response);
        });
    });
}
...
export function getUsersFromCache(start: number, end: number, excludedKey: string): Promise<IUserDocument[]> {
    return new Promise<any>((resolve, reject) => {
        client.zrange('user', start, end, (err, reply) => {
            if (err)  { reject(err) };
            ...
            multi.exec((error, replies) => {
                ...
                for (const reply of replies) {
                    ...
                    // reply.profilePicture = JSON.parse(reply.profilePicture);
                }
                resolve(replies);
            });
        });
...
export function updateUserFollowersInRedisCache(key: string, prop: string, value: number): Promise<void> {
    return new Promise((resolve, reject) => {
        client.hincrby(`users:${key}`, prop, value, (error) => {
            if (error) {
                reject(error);
            }
            resolve();
...
export function updateBlockedUserPropInRedisCache(key: any, prop: string, value: string, type: string): Promise<void> {
    return new Promise((resolve, reject) => {
